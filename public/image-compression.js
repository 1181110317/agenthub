...
//   apps/web/src/lib/imageCompression.ts  →  public/image-compression.js
// 改动（仅两处）：
//   1) TypeScript 类型擦除（TS → 浏览器脚本，UMD 尾部导出给 Node 测试）；
//   2) HEIC 转码由「动态 import("heic-to/csp")」改为可选：页面若加载了
//      heic-to 就用（window.heicTo），否则如实报 unreadable，不新增依赖。
// 算法（质量阶梯、回退降分辨率、像素上限、预算换算）与常量保持原样。
// 完整许可与署名见仓库根目录 THIRD-PARTY-NOTICES.md。
(function (root, factory) {
  const api = factory();
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  if (root) root.AHImage = api;
  ...
  const MAX_STASH_IMAGE_DATA_URL_CHARS = 1300000;
  /**
  ...
  const MAX_HEIC_DECODE_PIXELS = 64000000;
  ...
  function isHeicImageFile(file) {
    ...
    return (
      (file.type === '' || String(file.type).toLowerCase() === 'application/octet-stream') &&
      HEIC_IMAGE_EXTENSION.test(file.name)
    );
  }

  function findHeicMetadataBox(view, startOffset, endOffset, type) {
  ...
  async function validateHeicImageDimensions(file) {
    ...
    const containers = findHeicMetadataBox(view, properties.payloadOffset, properties.endOffset, 0x6970636f);
   ...
   */
  async function blobToDataUrl(blob, mimeTypeOverride) {
   ...
   */
  async function encodeCanvas(canvas, quality, mimeType, budgetChars) {
  ...
  function createComposerImageThumbnail(file) {
    ...
    const thumbnail = (async () => {
      ...
      try {
        ...
        return (await encodeCanvas(surface.canvas, 1, 'image/png', Number.POSITIVE_INFINITY))?.dataUrl ?? null;
      } catch {
        return null;
      } finally {
   ...
   */
  async function encodeWithinBudget(bitmap, maxDimension, budgetChars, preferredMimeType) {
    ...
    const mimeType =
      preferredMimeType ||
      ((await encodeCanvas(target.canvas, QUALITY_STEPS[0], 'image/webp', 0)) ? 'image/webp' : 'image/jpeg');
   ...
   */
  async function reencodeWithinBudget(file, budgetChars, preferredMimeType) {
    ...
    try {
      ...
      for (const dimensionScale of [1].concat(FALLBACK_SCALE_STEPS)) {
        ...
        try {
          ...
          // Keep trying the smaller fallback scales rather than giving up.
          encodeFailed = true;
          continue;
        }
        encodeFailed = false;
        if (encoded && encoded.dataUrl.length <= budgetChars) {
          return { ok: true, dataUrl: encoded.dataUrl, mimeType: encoded.mimeType, imageSize: encoded.imageSize };
        }
      }
  ...
  /**
   * Produces the payload to persist for a stashed image. Small images are
   * stored verbatim (preserving PNG transparency and exact pixels). Anything
   * over budget is downscaled and re-encoded.
   */
  async function compressImageForStash(file, budgetChars) {
    const budget = budgetChars == null ? MAX_STASH_IMAGE_DATA_URL_CHARS : budgetChars;
    let originalDataUrl;
    try {
    ...
    if (originalDataUrl.length <= budget) {
      return {
        ok: true,
        image: { dataUrl: originalDataUrl, mimeType: file.type, sizeBytes: file.size, recompressed: false },
      };
    }
    const reencoded = await reencodeWithinBudget(file, budget);
   ...
   */
  async function compressImageToByteLimit(file, maxBytes, options) {
    if (file.size <= maxBytes) return { ok: true, file, recompressed: false };
    const sourceSize = (options && options.sourceSizeBytes) != null ? options.sourceSizeBytes : file.size;
    if (sourceSize > MAX_COMPRESSIBLE_SOURCE_BYTES) return { ok: false, reason: 'too-large' };
    ...
    const reencoded = await reencodeWithinBudget(file, budgetChars, options && options.preferredMimeType);
    if (!reencoded.ok) return reencoded;
    return {
      ok: true,
      file: dataUrlToFile(
   ...
   * attachment size limit. Upstream loads a bundled decoder on demand; here the
   * decoder is optional (window.heicTo) so AgentHub keeps zero new dependencies
   * — without it, HEIC reports `unreadable` instead of silently failing.
   */
  async function prepareImageForAttachment(file, maxBytes) {
    ...
    try {
      ...
      if (typeof window === 'undefined' || typeof window.heicTo !== 'function') {
        return { ok: false, reason: 'unreadable' };
      }
      converted = await window.heicTo({ blob: file, type: 'image/jpeg', quality: QUALITY_STEPS[0] });
    } catch {
      return { ok: false, reason: 'unreadable' };
    }
    ...
    return result.ok ? Object.assign({}, result, { recompressed: true }) : result;
  }
