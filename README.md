...
A local web UI that brings multiple coding agents (Claude Code / Codex / ZCode / Gemini CLI / OpenCode / custom) into a single interface.
...
- Sidebar agent cards to switch between Claude Code / Codex / ZCode / Gemini CLI / OpenCode, each with its own session list
...
### 2. Provider / model management (read cc-switch)
- Auto-reads cc-switch's SQLite DB; claude / codex / gemini / openclaw providers are listed; keys are masked.
...
- Add providers manually (app-local only); balance queries (DeepSeek / OpenRouter / one-api family auto-detected).
...
### 5. Themes
- 🎨 menu switches 4 themes: **Garden Core / Ink Night / Paper White / Forest Eye-care**; xterm + mermaid palettes follow; preference remembered locally.
...
└─ data/              # Runtime data (sessions/usage/ssh/model-cache/settings)
```

## Notes

- Requires Node ≥ 22.5 (built-in `node:sqlite` for cc-switch DB). Local or WSL/SSH target must have the corresponding claude / codex / zcode CLI installed; remote native bridges check and upgrade on first connect per config.
- When cc-switch is running, its DB is opened read-only — no interference; on unreadable DB the UI surfaces a notice instead of silently showing an empty list.
