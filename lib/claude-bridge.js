...
function spawnSig(o) {
  return JSON.stringify([
    ...
    (o.remote && (o.remote.targetId || o.remote.label)) || '', !!o.nativeRemote,
  ]);
}
...
function armIdle(sess) {
  ...
  if (sess.pendingPerms.size) return; // 有待确认的权限/提问时不回收
...
function destroySession(key, reason) {
  ...
  if (sess.wslPidFile) {
    try {
      spawnSync('wsl.exe', ['-e', 'bash', '-c', `kill -9 -$(cat ${sess.wslPidFile}) 2>/dev/null; kill -9 $(cat ${sess.wslPidFile}) 2>/dev/null; rm -f ${sess.wslPidFile}`], { windowsHide: true, timeout: 8000 });
    } catch {}
  }
...
function runStreamTurn(o, emit) {
  ...
  if (sess && sess.sig !== spawnSig(o)) destroySession(key, 'sig-change');
  if (!sess) {
    sess = {
      ...
      cancelled: false, _resolve: null, emitter: () => {},
  ...
  const done = new Promise(resolve => {
    ...
    if (sess.busy) sess.queue.push(run); else run();
  });
  return {
    done,
    cancel: () => {
      ...
      for (const [rid] of sess.pendingPerms) {
        writeLine(sess, { type: 'control_response', response: { subtype: 'success', request_id: rid, response: { behavior: 'deny', message: '用户取消了本轮对话' } } });
...
function finishTurn(sess, code) {
  ...
  else if (sess._resolve) { const r = sess._resolve; sess._resolve = null; r(code); }
}
...
function spawnRemote(sess, o, args, settingsEnv, emit, cb) {
  ...
  if (o.cwd) parts.push(`cd ${shQuote(o.cwd)} 2>/dev/null || { echo "AgentHub: 远程工作目录不可用" >&2; exit 73; }`);
  const p = o.provider;
  let settingsFile = '';
  if (p) {
    const penv = HOOKS.providerEnv ? HOOKS.providerEnv(p) : (p.raw && p.raw.env) || {};
...
function handleLine(sess, line, emit) {
  ...
  if (obj.type === 'control_request' && obj.request) {
    ...
    if (req.subtype === 'can_use_tool') {
      ...
      const pid = obj.request_id;
      sess.pendingPerms.set(pid, { request: req, ts: Date.now() });
...
function respond(sessionKey, requestId, body) {
  ...
  const p = sess.pendingPerms.get(requestId);
  ...
  if (b.action === 'deny') {
    ...
    const ok = writeLine(sess, { type: 'control_response', response: { subtype: 'success', request_id: requestId, response: { behavior: 'allow', updatedInput: p.request.input, updatedPermissions: [p.request.permission_suggestions[b.suggestionIndex]] } } });
    armIdle(sess);
    return { ok };
  } else {
    response = { behavior: 'allow', updatedInput: p.request.input };
  }
  const ok = writeLine(sess, { type: 'control_response', response: { subtype: 'success', request_id: requestId, response } });
  armIdle(sess);
  return { ok };
}
