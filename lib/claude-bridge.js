...
function spawnSig(o) {
  return JSON.stringify([
    ...
    // 助手系统提示词是进程级参数：换了助手必须重启 CLI
    o.systemPrompt ? String(o.systemPrompt).slice(0, 64) : '',
  ]);
}
...
// Claude 会把不存在/已被清理的原生会话报告成 stderr，而不是一个结构化的
// stream-json error。这个 id 可能来自旧的 AgentHub 数据、导入文件，或 CLI
// 自己清理过的会话；继续把它当作普通进程崩溃会让网页永远卡在「出了问题」。
...
function clearSessionSettingsFile(sess) {
  if (!sess || !sess.settingsFile) return;
  try { fs.unlinkSync(sess.settingsFile); } catch {}
  sess.settingsFile = null;
}

// 续接启动阶段先暂存 stderr。这样失效 id 的诊断不会先落成一个红色助手
// 错误，随后又在同一回合里显示恢复后的正常回答。成功 init 或普通退出时再
// 一次性放出正常诊断。
function captureStderr(sess, text, emit) {
  const chunk = String(text || '');
  sess._stderrTail = String(sess._stderrTail || '').slice(-1200) + chunk;
  if (sess._resumeAttempt) {
    sess._pendingStderr = String(sess._pendingStderr || '').slice(-4000) + chunk;
    if (isMissingResumeError(sess._pendingStderr)) {
      sess._staleResumeDetected = true;
      sess._pendingStderr = '';
    }
    return;
  }
...
function flushPendingStderr(sess, emit) {
  const pending = String(sess._pendingStderr || '');
  sess._pendingStderr = '';
  if (pending && !sess._staleResumeDetected) emit({ kind: 'stderr', text: pending });
}

// 原生 resume 明确失败时，把本轮切换到网页历史回放，只重试一次。
// 原始用户消息仍保留在 AgentHub；CLI 收到的是 replayPrompt，因此不会丢掉
// 之前的上下文，也不会把失效的 cliSessionId 再写回 store。
function retryMissingResume(sess, emit) {
  if (!sess || !sess.busy || !sess._resumeAttempt || !sess._staleResumeDetected || sess._retriedStaleResume) return false;
  sess._retriedStaleResume = true;
  const old = sess.lastOpts || {};
  const retryOpts = { ...old, cliSessionId: '' };
  sess.cliSessionId = '';
  sess.lastOpts = retryOpts;
  if (sess.lastReplayUserMsg) sess.lastUserMsg = sess.lastReplayUserMsg;
  sess._gotResult = false;
  sess.gotInit = false;
  sess._resumeAttempt = false;
  sess._staleResumeDetected = false;
  sess._stderrTail = '';
  sess._pendingStderr = '';
  clearSessionSettingsFile(sess);
  emit({ kind: 'status', text: 'Claude 原生会话已失效，正在依据网页历史恢复…', resetCliSession: true });
  respawn(sess, retryOpts, emit, ok => {
    if (!ok || !writeLine(sess, sess.lastUserMsg)) {
      emit({ kind: 'error', text: 'CLI 启动失败' });
      finishTurn(sess, 1);
    }
  });
  return true;
...
async function startTurn(sess, o, emit, resolve) {
  ...
  sess._retriedStaleResume = false;
  sess._gotResult = false;
  sess._resumeAttempt = false;
  sess._staleResumeDetected = false;
  sess._stderrTail = '';
  ...
  sess.emitter = (ev) => {
    if (ev.kind === 'session' && ev.id) {
      sess.cliSessionId = ev.id;
      if (sess._resumeAttempt) {
        sess._resumeAttempt = false;
        flushPendingStderr(sess, emit);
      }
    }
    if (ev.kind === 'session') sess.gotInit = true;
    if (ev.kind === 'done' && sess._resumeAttempt && ev.isError && isMissingResumeError(ev.text)) {
      sess._staleResumeDetected = true;
      sess._gotResult = false;
      sess._pendingStderr = '';
      // 等 close 事件进入 retryMissingResume；不要把原生失败结果先渲染成红错。
      return;
    }
    if (ev.kind === 'done') {
      flushPendingStderr(sess, emit);
      sess._gotResult = true;
    }
  ...
  const replayContent = content.filter(b => b && b.type === 'image').map(b => ({ ...b }));
  replayContent.push({ type: 'text', text: typeof o.replayPrompt === 'string' && o.replayPrompt ? o.replayPrompt : o.prompt });
  sess.lastOpts = o; sess.lastUserMsg = userMsg;
  sess.lastReplayUserMsg = { type: 'user', message: { role: 'user', content: replayContent } };
...
function respawn(sess, o, emit, cb) {
  ...
  // 助手（lib/assistants.js）的系统提示词：用 CLI 原生追加，不往用户消息里塞内容
  args.push(...require('./assistants').claudeSystemArgs(o.systemPrompt, caps));
  const resumeId = sess.cliSessionId || o.cliSessionId;
  sess._resumeAttempt = !!resumeId;
  sess._staleResumeDetected = false;
  sess._stderrTail = '';
  sess._pendingStderr = '';
  ...
  // 注入型 MCP（本机）：把 AgentHub 的 git/额度/用量工具挂给 CLI（可用设置关闭）
  if (!o.wsl && !(o.remote && o.remote.exec) && HOOKS.mcpClaudeArgs) {
    try { args.push(...(HOOKS.mcpClaudeArgs(o) || [])); } catch (e) { console.error('[claude-bridge] mcp:', e.message); }
  }
...
function onChildClose(sess, code, emit, token) {
  ...
  sess.processToken = null;
  sess.child = null;
  if (wasBusy && retryMissingResume(sess, emit)) return;
  flushPendingStderr(sess, emit);
...
function spawnLocal(sess, o, args, env, emit, cb) {
  ...
  // localInvocation（lib/agents.js）把真正的参数塞进子进程环境变量、再用 cmd
  // 的延迟展开取出，shell:false + windowsVerbatimArguments，不经过 shell 拼接。
  // 不要在这里另造一份命令行字符串：cmd.exe 与 POSIX 的引号规则不同，
  // 拼错一处就是命令注入。
  const inv = HOOKS.localInvocation(bin, args, env);
  ...
  child.stderr.on('data', d => {
    ...
    captureStderr(sess, d.toString('utf8'), emit);
  });
...
function spawnWSL(sess, o, args, settingsEnv, emit, cb) {
  ...
  const feed = wireLineStream(sess, emit);
  child.stdout.on('data', d => { if (sess.processToken === token) feed(d); });
  child.stderr.on('data', d => { if (sess.processToken === token) captureStderr(sess, d.toString('utf8'), emit); });
...
function spawnRemote(sess, o, args, settingsEnv, emit, cb) {
  ...
  proc.onStderr(t => { if (sess.processToken === token) captureStderr(sess, t, emit); });
  proc.done.then(code => {
    if (sess.processToken !== token) return;
    const wasBusy = sess.busy;
    sess.processToken = null;
