...
function detectAgents(settings) {
  ...
  for (const id of Object.keys(DEFS)) {
    const def = DEFS[id];
    if (id === 'builtin') continue; // 按需求不在列表展示（引擎保留，ACP 连接器是外部 Agent 的正路）
    const bin = resolveBin(id, agentSettings(settings, id));
    let version = '';
    if (bin) {
      try {
    ...
    out.push({ id, name: def.name, color: def.color, style: def.style, models: def.models, bin, found: !!bin, version });
  }
  // 用户自定义 agent
...
async function detectAgentsAsync(settings) {
  ...
  for (const id of Object.keys(DEFS)) {
    const def = DEFS[id];
    if (id === 'builtin') continue; // 不在列表展示
    jobs.push((async () => {
      const bin = await resolveBinAsync(id, agentSettings(settings, id));
      let version = '';
      if (bin) { try { version = (await execCliP(bin, ['--version'])).split(/\r?\n/)[0] || ''; } catch {} }
      return { id, name: def.name, color: def.color, style: def.style, models: def.models, bin, found: !!bin, version };
    })());
  }
  for (const c of (settings && settings.customAgents) || []) {
    jobs.push((async () => {
...
async function preWarm(settings) {
  for (const id of Object.keys(DEFS)) {
    ...
    if (def.style === 'codex') {
      try { codexResumeSupportsImage(bin); } catch {}
    }
  }
...
function codexResumeSupportsImage(bin) {
  ...
  try {
        ...
        const r = spawnSync(inv.bin, inv.args, { encoding: 'utf8', timeout: 25000, shell: inv.shell, windowsVerbatimArguments: inv.windowsVerbatimArguments, env: inv.env || process.env, windowsHide: true });
...
function buildCodexArgs(o, bin) {
  ...
  const imgs = o.images || [];
  const canAttach = imgs.length && (!o.cliSessionId || codexResumeSupportsImage(bin));
  if (canAttach) {
...
module.exports = {
  DEFS, runAgent, detectAgents, detectAgentsAsync, preWarm, resolveBin, resolveBinAsync, agentSettings,
