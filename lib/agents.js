...
const { requireLocalDirectory } = require('./workspace');
...
const mcpInject = require('./mcp-inject');
const { DATA_DIR } = require('./store');

// 注入型 MCP（AgentHub → CLI agent）：port/token 由 server 注入，enabled 在
// 每次拉起 CLI 时求值（设置改动即时生效）。默认开启。
let _mcpOptions = null;
function setMcpOptions(opts) { _mcpOptions = opts || null; }
function _mcpArgs(o, kind) {
  if (!_mcpOptions) return [];
  try {
    const dir = path.join(DATA_DIR, 'tmp-settings');
    const disabled = typeof _mcpOptions.disabled === 'function' ? _mcpOptions.disabled() : _mcpOptions.disabled;
    const agenthubOn = !(typeof _mcpOptions.enabled === 'function' && !_mcpOptions.enabled());
    const agentId = kind === 'codex' ? 'codex' : 'claude';
    // 第三方 MCP 服务器独立于「AgentHub 注入工具」总开关：关掉自带工具
    // 不应该连带禁用用户自己配的服务器。
    const extra = typeof _mcpOptions.extraServers === 'function' ? _mcpOptions.extraServers(agentId) : null;
    const ctx = {
      port: _mcpOptions.port, token: _mcpOptions.token, sessionId: o && o.sessionKey || '',
      disabled: agenthubOn ? disabled : null, agenthubOff: !agenthubOn,
      extra: extra && extra.claude ? extra.claude : null,
    };
    if (kind === 'codex') {
      const base = agenthubOn ? mcpInject.codexArgs(ctx) : [];
      return base.concat(extra && Array.isArray(extra.codex) ? extra.codex : []);
    }
    const file = mcpInject.writeClaudeConfig({ ...ctx, dir });
    return file ? ['--mcp-config', file] : [];
  } catch (e) {
    console.error('[agents] mcp inject failed:', e.message);
    return [];
  }
}
...
const DEFS = {
  claude: {
    ...
    // Claude CLI 的 --model 帮助中明确列出 fable；其余为常用别名。
    models: ['fable', 'sonnet', 'opus', 'haiku', 'opusplan'],
  },
  codex: {
    id: 'codex', name: 'Codex', style: 'codex', color: '#34b189',
    binNames: ['codex'],
    winPaths: ['%LOCALAPPDATA%\\OpenAI\\Codex\\bin\\codex.cmd'],
    // Codex CLI 会随版本更新模型目录；这些是 CLI 不可用时的兜底项。
  ...
  for (const id of Object.keys(DEFS)) {
    ...
    if (bin && !virtual) {
      try {
        ...
        version = parseCliVersion((r.stdout || '') + (r.stderr || ''));
        ready = r.status === 0 && !!version;
      } catch {}
    }
    out.push({ id, name: def.name, color: def.color, style: def.style, models: def.models, bin, found: ready, ready, pathFound: !!bin, version, probeError: bin && !ready ? 'CLI 未返回有效版本号或启动失败' : '', virtual });
  }
  // 用户自定义 agent
...
const execCliResult = (bin, args = [], timeout = 12000) => new Promise(resolve => {
  ...
  const finish = code => {
    ...
    settled = true;
    clearTimeout(timer);
    resolve({ code: Number.isFinite(Number(code)) ? Number(code) : 1, output: output.trim(), error, timedOut });
  };
  try {
    ...
    child.once('error', e => { error = e; finish(1); });
    child.once('close', code => { exitCode = Number(code); finish(exitCode); });
    timer = setTimeout(() => {
      timedOut = true;
      error = new Error('CLI 探测超时');
      try { if (!killTree(child.pid)) child.kill(); } catch {}
      finish(124);
    }, timeout);
    if (timer.unref) timer.unref();
  } catch (e) { error = e; finish(1); }
});
const execCliP = (bin, args = [], timeout = 12000) => execCliResult(bin, args, timeout).then(r => r.output);

function parseCliVersion(output) {
  const lines = String(output || '').split(/\r?\n/).map(x => x.trim()).filter(Boolean);
  return lines.find(line => !/^(?:node:|internal\/|Error:|SyntaxError|at\s|MODULE_NOT_FOUND|Access is denied)/i.test(line)
    && /(?:\d+\.\d+(?:\.\d+)?|version|claude|codex|gemini|opencode|zcode)/i.test(line)) || '';
}

function cliProbeError(result, version) {
  if (version) return '';
  if (result && result.timedOut) return 'CLI 探测超时';
  if (result && result.error) {
    const code = result.error.code ? `（${result.error.code}）` : '';
    return 'CLI 无法启动' + code + (result.error.message ? '：' + result.error.message : '');
  }
  const first = String(result && result.output || '').split(/\r?\n/).map(x => x.trim()).find(Boolean);
  if (first) return 'CLI 探测失败：' + first.slice(0, 240);
  return 'CLI 未返回有效版本号';
}
// Codex 的可用模型由本机 CLI 的模型目录决定，不能长期依赖写死列表。
// --bundled 只读取当前安装包自带的目录，不触发网络刷新，也不会修改用户配置。
function parseCodexModelCatalogEntries(output) {
  try {
    const root = JSON.parse(String(output || ''));
    const rows = Array.isArray(root) ? root : (root && Array.isArray(root.models) ? root.models : []);
    const seen = new Set();
    return rows
      .filter(m => m && (m.visibility == null || m.visibility === 'list') && m.supported_in_api !== false)
      .map(m => {
        ...
        return {
          id,
          label: String(m.display_name || '').trim(),
          description: String(m.description || '').trim().slice(0, 500),
          contextWindow: Number(m.context_window || m.max_context_window) || 0,
          maxOutputTokens: Number(m.max_output_tokens) || 0,
          reasoningLevels: Array.isArray(m.supported_reasoning_levels)
            ? m.supported_reasoning_levels.map(x => x && x.effort).filter(Boolean)
            : [],
          inputModalities: Array.isArray(m.input_modalities) ? m.input_modalities : [],
          supportsTools: !!(m.tool_mode || m.supports_search_tool || (Array.isArray(m.experimental_supported_tools) && m.experimental_supported_tools.length)),
          supportsImages: Array.isArray(m.input_modalities) ? m.input_modalities.includes('image') : undefined,
          supportsImageDetailOriginal: m.supports_image_detail_original === true,
        };
      })
      .filter(Boolean);
  } catch {
...
function parseCodexModelCatalog(output) {
  return parseCodexModelCatalogEntries(output).map(m => m.id);
}
async function discoverCodexModels(bin) {
  if (!bin) return [];
  const output = await execCliP(bin, ['debug', 'models', '--bundled'], 10000);
  return parseCodexModelCatalog(output);
}
async function discoverCodexModelCatalog(bin) {
  if (!bin) return [];
  const output = await execCliP(bin, ['debug', 'models', '--bundled'], 10000);
  return parseCodexModelCatalogEntries(output);
}
// 能力位（P2-A）：把服务端探测到的 CLI 能力如实广播给前端，前端按能力渲染
// UI（能力必须如实广告——不能回滚就不显示回滚按钮，而不是点了再报错）。
// claude 系用 --help 探测旗标；zcode/codex 的 app-server 原生桥能力由
// native-runtime 探测，这里一并透出已知结论；探测不到就少给，宁缺勿假。
function deriveFeatures(id, style, bin, caps) {
  const c = caps || {};
  if (style === 'claude') {
    return {
      streamBridge: !!c.streamInput,        // 常驻双向流式桥（--input-format stream-json）
      // --permission-prompt-tool 是隐藏旗标（--help 探测不到，BUGS C1），但
      // stream-json 模式下默认启用、运行时探测降级兜底——如实跟随 streamInput
      permBridge: !!c.streamInput,
      partial: !!c.partial,                 // token 级增量直播
      settingsFlag: !!c.settings,           // --settings 供应商覆盖
      permMode: !!c.permMode,               // --permission-mode
    };
  }
  if (id === 'zcode' || id === 'codex') {
    // app-server 原生协议：本机安装即可用；远程/WSL 由 prepareNativeRoute 运行时判定
    return { appServer: true, streamBridge: true, permBridge: true, partial: true };
  }
  if (id === 'builtin') return { streamBridge: true, partial: true, tools: true };
  return {}; // raw / gemini / opencode / 自定义：不假装支持
}
async function detectAgentsAsync(settings) {
  ...
  for (const id of Object.keys(DEFS)) {
    ...
    jobs.push((async () => {
      ...
      let modelCatalog = [];
      if (bin && !virtual) {
        probe = await execCliResult(bin, ['--version'], 12000);
        version = parseCliVersion(probe.output);
        if (def.style === 'claude') {
          try { caps = await cliCapsAsync(bin); } catch { caps = null; }
        }
        if (id === 'codex') {
          try {
            modelCatalog = await discoverCodexModelCatalog(bin);
            if (modelCatalog.length) models = modelCatalog.map(m => m.id);
          } catch {}
        }
      }
      const ready = virtual || (!!bin && !!probe && probe.code === 0 && !!version);
      return {
        id, name: def.name, color: def.color, style: def.style, models, modelCatalog, bin,
        // found 保持“可运行”语义；pathFound 单独告诉界面“只找到了路径”。
        // 这样像 ZCode .cjs 启动失败时不会显示成 CLI 就绪。
        found: ready, ready, pathFound: !!bin, version,
        probeError: bin && !ready ? cliProbeError(probe, version) : '',
        features: deriveFeatures(id, def.style, bin, caps),
      };
    })());
  }
  for (const c of (settings && settings.customAgents) || []) {
    jobs.push((async () => {
      ...
      return { id: c.id, name: c.name, color: c.color || '#94a3b8', style: 'raw', models: c.models || [], modelCatalog: [], bin, found: !!bin, ready: !!bin, pathFound: !!bin, version: '', custom: true, features: {} };
    })());
  }
...
function buildClaudeEnv(o, bin) {
  ...
  if (o.effort) {
    const map = { minimal: '0', low: '8192', medium: '16384', high: '31999', xhigh: '31999', max: '31999', ultra: '31999' };
...
function buildCodexEnv(o) {
  ...
  if (p && !o.remote && (praw.config || praw.auth || p.baseUrl || p.apiKey)) {
    ...
    const home = path.join(DATA_DIR, 'codex-homes', id);
...
async function runAgent(o, emit) {
  ...
  const rawDecoder = style === 'raw' ? new StringDecoder('utf8') : null;
  const feed = (chunk) => {
    const buf = Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk), 'utf8');
    if (style === 'raw') { const decoded = rawDecoder.write(buf); if (decoded) emit({ kind: 'text', text: decoded }); return; }
  ...
  const feedEnd = () => {
    if (style === 'raw') { const tail = rawDecoder.end(); if (tail) emit({ kind: 'text', text: tail }); }
    else if (lineBuf.length) { try { parser(lineBuf.toString('utf8')); } catch {} }
...
// Windows OpenSSH 主机的默认 shell 可能是 cmd 或 PowerShell。把完整脚本
// 编码后再交给 powershell.exe，避免路径、API Key 和模型参数被外层 shell
// 二次解释；CLI 进程继承 SSH 通道的 stdin/stdout，因此仍可走一次性协议。
...
function powershellInvocation(bin, args) {
  const words = nativeRuntime.commandWords(bin);
  return '& ' + [...words, ...(args || [])].map(powershellLiteral).join(' ');
}
function powershellEnv(parts, values) {
  for (const [key, value] of Object.entries(values || {})) {
    if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(key) && typeof value === 'string') {
      parts.push('$env:' + key + ' = ' + powershellLiteral(value));
    }
  }
}

function windowsRemoteCodexParts(provider) {
  if (!provider) return [];
  const raw = providerRawConfig(provider);
  const values = providerEnv(provider);
  const parts = [];
  powershellEnv(parts, values);
  const id = String(provider.ccsId || provider.id || 'provider').replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 60) || 'provider';
  let config = typeof raw.config === 'string' ? raw.config : '';
  let auth = raw.auth;
  const apiKey = providerApiKey(provider);
  ...
  if (!auth && apiKey) auth = { OPENAI_API_KEY: apiKey };
  if (config || auth) {
    const rel = '.agenthub-codex\\' + id;
    parts.push('$agentHubCodexHome = Join-Path $env:USERPROFILE ' + powershellLiteral(rel));
    parts.push('New-Item -ItemType Directory -Force -LiteralPath $agentHubCodexHome | Out-Null');
    if (config) {
      const encoded = Buffer.from(config, 'utf8').toString('base64');
      parts.push('[IO.File]::WriteAllText((Join-Path $agentHubCodexHome \'config.toml\'), [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String(' + powershellLiteral(encoded) + ')))');
    }
    if (auth) {
      const encoded = Buffer.from(typeof auth === 'string' ? auth : JSON.stringify(auth), 'utf8').toString('base64');
      parts.push('[IO.File]::WriteAllText((Join-Path $agentHubCodexHome \'auth.json\'), [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String(' + powershellLiteral(encoded) + ')))');
    }
    parts.push('$env:CODEX_HOME = $agentHubCodexHome');
  } else if (apiKey && !values.OPENAI_API_KEY && !values.CODEX_API_KEY) {
    parts.push('$env:OPENAI_API_KEY = ' + powershellLiteral(apiKey));
  }
  return parts;
}

function runRemoteWindows(o, feed, feedEnd, emit) {
  const { args, stdinPrompt = true } = buildCommand(o);
  const bin = (o.custom && o.custom.bin)
    || ((!o.custom && nativeRuntime.remoteBin(o, o.agent)) || ((DEFS[o.agent] || {}).binNames || [o.agent])[0]);
  const style = o.custom ? 'raw' : ((DEFS[o.agent] || {}).style || 'raw');
  const parts = [
    // Windows PowerShell 5 默认使用本地代码页；先把 PowerShell/native
    // stdout 设为 UTF-8，避免 Codex JSON 里的中文也被远端 SSH 转坏。
    ...
    "$agentHubPath = @()",
    "if ($env:USERPROFILE) { $agentHubPath += (Join-Path $env:USERPROFILE '.local\\bin') }",
    "if ($env:APPDATA) { $agentHubPath += (Join-Path $env:APPDATA 'npm') }",
    'if ($env:Path) { $agentHubPath += $env:Path }',
    "$env:Path = ($agentHubPath | Where-Object { $_ } | Select-Object -Unique) -join ';'",
  ];
  if (o.cwd && o.cwd !== '~') {
    parts.push('try { Set-Location -LiteralPath ' + powershellLiteral(o.cwd) + ' -ErrorAction Stop } catch { [Console]::Error.WriteLine(\'AgentHub: Windows 远程工作目录不可用\'); exit 73 }');
  } else {
    parts.push('try { Set-Location -LiteralPath $env:USERPROFILE -ErrorAction Stop } catch { exit 73 }');
  }

  const p = o.provider;
  if (style === 'zcode') {
    powershellEnv(parts, zcodeEnvOverrides(o));
  } else if (style === 'codex' && p) {
    parts.push(...windowsRemoteCodexParts(p));
    emit({ kind: 'status', text: '已映射 Codex 供应商配置到 Windows 远程环境' });
  } else if (p) {
    const env = {};
    if (p.baseUrl) env.ANTHROPIC_BASE_URL = p.baseUrl;
    const apiKey = providerApiKey(p);
    if (apiKey) env.ANTHROPIC_AUTH_TOKEN = apiKey;
    Object.assign(env, providerEnv(p));
    delete env.ANTHROPIC_API_KEY;
    powershellEnv(parts, env);
  }
  const eff = effortExport(o);
  if (eff && style === 'claude') {
    const at = eff.indexOf('=');
    if (at > 0) powershellEnv(parts, { [eff.slice(0, at)]: eff.slice(at + 1) });
  }

  parts.push('$agentHubExit = 0');
  parts.push('try { ' + powershellInvocation(bin, args) + '; if ($LASTEXITCODE -ne $null) { $agentHubExit = [int]$LASTEXITCODE } } catch { [Console]::Error.WriteLine($_.Exception.Message); $agentHubExit = 1 }');
  parts.push('exit $agentHubExit');
  const command = powershellEncodedCommand(parts.join('\n'));
  emit({ kind: 'status', text: 'ssh ' + (o.remote.label || '') + ' $ ' + bin + ' ' + args.join(' ') });
  const proc = o.remote.exec(command, stdinPrompt ? o.prompt : '');
  proc.onStdout(feed);
  ...
  const done = proc.done.then(code => {
    ...
    const exitCode = Number.isFinite(n) ? n : 1;
    feedEnd();
    if (exitCode === 127) emit({ kind: 'error', text: 'Windows 远程主机上找不到 ' + bin + '：请确认 CLI 已安装并在 PATH 中，或在 Agent 设置里填写 remoteBin' });
...
function isMissingClaudeResumeError(text) {
  return /no\s+conversation\s+found\s+with\s+session\s+id|conversation\s+not\s+found/i.test(String(text || ''));
}

function runLocal(o, feed, feedEnd, emit) {
  const { bin, args, env, stdinPrompt = true } = buildCommand(o);
  const style = o.custom ? 'raw' : ((DEFS[o.agent] || {}).style || 'raw');
  const resumeAttempt = style === 'claude' && !!o.cliSessionId && o._claudeResumeRetried !== true;
  ...
  if (stdinPrompt) child.stdin.write(o.prompt);
  child.stdin.end();
  let cancel = () => { const ok = killTree(child.pid); if (!ok) { try { child.kill(); } catch {} } };
  const done = new Promise(resolve => {
    child.on('close', (code) => {
      const n = Number(code);
      const exitCode = Number.isFinite(n) ? n : 1;
      if (resumeAttempt && isMissingClaudeResumeError(stderrTail)) {
        emit({ kind: 'status', text: 'Claude 原生会话已失效，正在依据网页历史恢复…', resetCliSession: true });
        const retry = runLocal({
          ...o,
          cliSessionId: '',
          prompt: typeof o.replayPrompt === 'string' && o.replayPrompt ? o.replayPrompt : o.prompt,
          _claudeResumeRetried: true,
        }, feed, feedEnd, emit);
        cancel = retry.cancel;
        retry.done.then(resolve, () => resolve(1));
        return;
      }
      if (stderrTail) emit({ kind: 'stderr', text: stderrTail });
      feedEnd(); emit({ kind: 'exit', code: exitCode }); resolve(exitCode);
    });
  });
  return {
...
function runRemote(o, feed, feedEnd, emit) {
  if (o.remote && o.remote.platform === 'windows') return runRemoteWindows(o, feed, feedEnd, emit);
  const { args, stdinPrompt = true } = buildCommand(o);
...
bridge.initBridge({
  ...
  localInvocation,
  mcpClaudeArgs: o => _mcpArgs(o, 'claude'),
});
...
codexBridge.initBridge({
  ...
  remoteBin: nativeRuntime.remoteBin,
  winToWsl,
  mcpCodexArgs: o => _mcpArgs(o, 'codex'),
});
...
module.exports = {
  ...
  prepareNativeRoute, clearNativeRouteCache, localInvocation, discoverCodexModels, discoverCodexModelCatalog,
  setMcpOptions,
};
