...
function aggregate({ days = 30, agent = 'all', source = 'all' } = {}) {
  ...
  const byDay = new Map(), byModel = new Map(), byAgent = new Map(), byProvider = new Map(), bySource = new Map();
  const details = [];
  const totals = blank();
  ...
  for (const r of filtered) {
    ...
    for (const bucket of [d, m, a, p, src, totals]) addCounters(bucket, values);
    ...
    details.push({
      ...
      sessionId: safeText(r.sessionId, '', 256), ...values,
      tokens: tokenTotal(values),
    });
    byDay.set(r.date, d); byModel.set(model, m); byAgent.set(agentName, a);
    byProvider.set(providerName, p); bySource.set(sourceName, src);
  }
