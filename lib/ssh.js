...
function listHosts() {
  return sshStore.data.hosts.map(h => ({ ...h, password: h.password ? '********' : '', privateKey: h.privateKey ? '(已存)' : '' }));
}
...
function execStream(cfg, command, stdinData) {
  ...
  handle.done = connect(cfg).then(conn => new Promise((resolve, reject) => {
    ...
    conn.exec(command, { pty: false }, (err, stream) => {
      ...
      let code = 0;
      stream.on('data', d => handle._out(d.toString('utf8')));
      stream.stderr && stream.stderr.on('data', d => handle._err(d.toString('utf8')));
...
module.exports = { listHosts, getHostCfg, saveHost, deleteHost, execStream, openShell, testHost, listRemoteDirs };
