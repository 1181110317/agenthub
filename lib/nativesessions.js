...
function findCodexFile(sessionId) {
  ...
  const roots = [
    ...
    path.join(DATA_DIR, 'codex-homes'),
  ];
  const files = [];
...
function truncateNativeSession(agent, cliSessionId, mode) {
  ...
  const out = lines.slice(0, cut).map((line, i) => {
    const event = objs[i];
    if (!event) return line;
    let changed = false;
    if (event.sessionId === cliSessionId) { event.sessionId = newId; changed = true; }
    if (isCodex && event.type === 'session_meta' && event.payload) {
      for (const key of ['id', 'session_id']) {
        if (event.payload[key] === cliSessionId) { event.payload[key] = newId; changed = true; }
      }
    }
