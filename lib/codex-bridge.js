...
function spawnProcess(sess, o) {
  ...
  // 注入型 MCP（本机）：-c mcp_servers.agenthub.* 把 AgentHub 工具挂给 Codex
  if (HOOKS.mcpCodexArgs) {
    try { args.push(...(HOOKS.mcpCodexArgs(o) || [])); } catch (e) { console.error('[codex-bridge] mcp:', e.message); }
  }
